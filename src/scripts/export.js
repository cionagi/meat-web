...
function hoistPages(fileExt, outputPath) {
  console.log(`${nextjsConfig.distDir}/server/static/${BUILD_ID}/pages/**/*${fileExt} -> ${outputPath}/`)
  shell.mkdir('-p', outputPath)
  var match = new RegExp('\\' + `${fileExt}`)
  var filesToHoist = shell.find(`${nextjsConfig.distDir}/server/static/${BUILD_ID}/pages/`).filter(function (file) {
...
console.log('Hoist public/ Next.js runtime and optimised chunks, computed .html and .json data\n')
